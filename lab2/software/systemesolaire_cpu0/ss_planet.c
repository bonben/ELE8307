...
inline int  ss_planet_getBall3DReferenceIntensity( float x, float y, float z, int radius ) {
	...
	float r_2, r_over_R_2;

	const float r2_to_intensity[101] =
	{
	    30, 30, 30, 30, 29, 29, 29, 29, 29, 29, 28, 28, 28, 28, 28, 28, 27,
	    27, 27, 27, 27, 27, 26, 26, 26, 26, 26, 26, 25, 25, 25, 25, 25, 25,
	    24, 24, 24, 24, 24, 23, 23, 23, 23, 23, 22, 22, 22, 22, 22, 21, 21,
	    21, 21, 21, 20, 20, 20, 20, 19, 19, 19, 19, 18, 18, 18, 18, 17, 17,
	    17, 17, 16, 16, 16, 16, 15, 15, 15, 14, 14, 14, 13, 13, 13, 12, 12,
	    12, 11, 11, 10, 10,  9,  9,  8,  8,  7,  7,  6,  5,  4,  3,  0,
	};
    ...
    r_2 = x*x + y*y;
    r_over_R_2 = r_2 / (float)(radius * radius);
    r_over_R_2 *= 100.0f; // scale on 100 points
    r_over_R_2 += 0.5f;   // shift for rounding
    
    if( z <= 0 )
    	intensity = r2_to_intensity[(int)r_over_R_2];
    else
